...
#         swam_weights = outputs.attentions.argmax(dim=-1)
#         modified_weights = outputs.attentions
#         modified_weights[range(modified_weights.size(0)), swam_weights] = -10
#         modified = model(**inputs, output_attentions=True, force_swam_weight=modified_weights)
#         modified_logits = modified.logits
#         modified_labels = modified_logits.argmax(dim=-1)
#         modified_success = (modified_labels == 1 - pred_labels).sum().item()
#         total_success += modified_success
# print(total_success / len(validation_dataset))
# %%
total_success = 0
loss_func = torch.nn.CrossEntropyLoss()
for i in range(len(validation_dataset)):
    tmp_input = validation_dataset[i]
...
print(f"Success edit rate: {total_success/len(validation_dataset)}")