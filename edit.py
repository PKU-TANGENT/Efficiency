...
for ii in range(len(to_predict)):
    ...
    plt.bar(get_unique_words(tokenizer.convert_ids_to_tokens(inputs["input_ids"][ii])),outputs.attentions[ii].cpu().detach().numpy())
    plt.bar(get_unique_words(tokenizer.convert_ids_to_tokens(inputs["input_ids"][ii])),(outputs.attentions[ii].where(outputs.attentions[ii]>1/torch.sum(inputs["attention_mask"][ii]),torch.zeros_like(outputs.attentions[ii]))).cpu().detach().numpy())
