...
prefix="lora-freeze-"
learning_rate=2e-3
suffix="-${pooler_type}-layer${lora_layers}"
...
# python -m debugpy --listen 127.0.0.1:9999 --wait-for-client lora_glue.py \
python lora_glue.py \
  --task_name $TASK_NAME \
  ...
  --freeze_backbone \
  --model_class_name "Lora${model_architecture^}ForSequenceClassification" \
  --model_package_name "modeling_lora_${model_architecture}" \
